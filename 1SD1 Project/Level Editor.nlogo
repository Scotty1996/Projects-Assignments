...
globals [
  ...
  dead?         ;; True when player is loses a life
...
to draw
  ...
  [
    ...
    ;; Wall Tool - Draws a Wall
    if tool = "Draw Wall"
    [ draw-boundary gray ]
    ;; Gate Tool - Draws Gate
    if tool = "Draw Gate"
    [ draw-boundary brown ]
    ;; flag Tool
    if tool = "Place Flag"
    ...
    [ draw-boundary cyan ]
     ;; Place lava
    if tool = "Draw Lava"
    [ draw-boundary orange ]
    ;; Place Marsh
    if tool = "Draw Marsh"
    [ draw-boundary 53 ]

    ;; Player Tool
    if tool = "Place Player"
    [ place-player ]
    ;; Monster Tool
    if tool = "Place Monster"
    [ place-monster ]
    ;; ;;npc Tool
    if tool = "Place Enemy"
    [ place-enemy ]
...
to draw-boundary [ boundary-color ]
  ...
  [
    ...
    [
      ...
      [
        ifelse any? players-here
        [ user-message "You cannot place a gate on top of player or a pellet." ]
        [ set pcolor boundary-color ]
      ]
      [ user-message "You cannot place a wall on top of player, a ghost, or a pellet." ]
    ]
  ]
end
...
to place-flag
  ...
  [ user-message "You must place flag on a corridor space, not a wall or a gate." ]
  [
    ask flags
...
to place-player
  ifelse [pcolor] of patch round mouse-xcor mouse-ycor != black
  [ user-message "You must place player on a corridor space, not a wall or a gate." ]
  [
    ask players
    [ setxy (round mouse-xcor) (round mouse-ycor) ]
  ]
...
;; Place monster
to place-monster
  ifelse [pcolor] of patch mouse-xcor mouse-ycor != black
  [ user-message "You must place player on a corridor space, not a wall or a gate." ]
  [
      ask one-of monsters
      [ setxy (round mouse-xcor) (round mouse-ycor) ]
...
to place-enemy
  ...
  [ user-message "You must place player on a corridor space, not a wall or a gate." ]
  [
     ask one-of enemys with [ color = item (which-enemy - 1) enemy-colors ]
       [ setxy (round mouse-xcor) (round mouse-ycor) ]
  ]
end
...
to save-level
  ...
  let filepath (word "../playermap" level ".csv")
...
to load-level
  ...
  let filepath (word "playermap" choice ".csv")
