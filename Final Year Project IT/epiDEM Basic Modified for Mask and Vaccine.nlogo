;; r0 of SARS is 0.19-1.08 source: https://www.news-medical.net/health/What-is-R0.aspx
;; r0 of SARS is 2.4 source: https://www.thelancet.com/journals/laninf/article/PIIS1473-3099(20)30484-9/fulltext
;; Adequate mask protection offers atleast 80% effectiveness "against clinical influenza-like illness" source1: https://www.ijidonline.com/article/S1201-9712(08)01008-4/fulltext
;; Source2: https://www.weforum.org/agenda/2020/04/should-we-be-promoting-the-widespread-use-of-masks/#:~:text=Research%20shows%20masks%20are%20effective%20as%20stopping%20the,has%20emerged%20in%20the%20face%20of%20this%20pandemic.

;; Set infection-chance to 50% or 50/100 for fair probability of contracting infection.
;; Set mask-effectiveness to 0% or 0/100 for no protective measure against contracting infection.
;; Set mask-effectiveness to 80% or 80/100 for minimum adequate protection against contracting infection.

;; Based on above sources, SARS infection has an average r0 of 1.2233 and range of 0.19-2.4.
;; r0 average is 0.19 + 1.08 + 2.4 = 3.67 / 2 = 1.2233...

;; SARS has a typical incubation period of 2-7 days, possibly 10 days and rarely 14 days in which the infection cannot be transmitted, source: https://www.cdc.gov/sars/about/faq.html
;; SARS, once symptoms show, infection can be transmitted, and 10 days should be allowed to pass from onset of symptoms to be considered non infectious, source: https://www.cdc.gov/sars/about/faq.html
;; Based on these numbers, SARS has a minimum duration of 12 days, average duration of 18 days and maximum duration of 24 days.
;; Therefore, based on the above durations of SARS infection, the minimum and maximum average-recovery-time range has been adjusted to reflect this.

;; SARS has an estimated death rate of 14%-15%, source: https://www.who.int/csr/sars/archive/2003_05_07a/en/#:~:text=On%20the%20basis%20of%20more%20detailed%20and%20complete,estimate%20of%20case%20fatality%20of%2014%25%20to%2015%25.
;; SARS has an estimated death rate of 10%, source: https://jamanetwork.com/journals/jama/fullarticle/2759815
;; Death rate average is 15% + 10% = 25% / 2 = 12.5%.
;; Based on this, the survival rate is between 85%-90% with an average of 87.5%.
;; Recovery rate average is 85% + 90% = 175 / 2 = 87.5%.
;; Therefore, the recovery-chance range has been adjusted to reflect this.

;; This model is a modified verson of the Mask model and fuctions near-identically
;; This model asumes the vaccine is always effective.
;; SARS-CoV-1 strain has no known vaccine, this model is interrested only in analysis r0 and related trends.
...
[
  ...
  vaccine?            ;; If true, the person is vaccinated
]
...
to setup
  clear-all
  setup-people
  setup-people-mask ;; Setup People who wear masks
  setup-people-vaccine ;; Setup People who are vaccinated
  reset-ticks
end
...
to setup-people
  ...
  [
    ...
    set mask? false ;; These people do not wear masks
    set vaccine? false ;; These people are not vaccinated
...
;; Setup for people who wear masks
;; Mask people have a chance at starting off infected
to setup-people-mask
  create-turtles initial-people-mask
  [
   setxy random-xcor random-ycor
    set cured? false
    set infected? false
    set susceptible? true
    set mask? true ;; These people wear masks
    set vaccine? false ;; These people are not vaccinated
...
;; Setup for people who are vaccinated
to setup-people-vaccine
  create-turtles initial-people-vaccine
  [
   setxy random-xcor random-ycor
    set cured? false
    set infected? false
    set susceptible? true
    set mask? false ;; These people do not wear masks
    set vaccine? true ;; These people are vaccinated
...
;; Blue is a person wearing a mask
...
to go
  if all? turtles [ not infected? ]
    [ export-all-plots "C:/Users/Scott/Desktop/ProjectData Vaccine & Mask/Test.csv"
      stop ]
...
;; If infected is wearing mask
to mask-infect

  let nearby-uninfected (turtles-on neighbors)
     with [ not infected? and not cured? ]

    ;; If person is wearing mask, effects of pathogen decreased in both directions
    ;; If both wear mask, infection is further mitigated.
    ;; If mask offers 80% effectiveness and 20% risk for both turtles,
    ;; Then the effectiveness and risk must increase and decrease proportionally,
    ;; Example: Mask offers 80% effectiveness, therefor risk of 20% is decreased by 80%
    ;; 20 - 80% = 4% ( New Risk ) or 80 + 20% = 96% ( New effectiveness )


    if nearby-uninfected != nobody
     [ ask nearby-uninfected
       [ if mask? = true
        [ if random-float 100 > (mask-effectiveness * 1.2) ;; Infect mask-mask true
         [ set infected? true
           set nb-infected (nb-infected + 1)
         ]
       ]

        if mask? = false and vaccine? = false
        [ if infection-chance > mask-effectiveness ;; Infect mask-no mask true
         [ set infected? true
           set nb-infected (nb-infected + 1)
         ]
       ]
...
;; If infected is not wearing a mask
to infect-no-mask
  ...
  let nearby-uninfected (turtles-on neighbors)
    ...
    if nearby-uninfected != nobody
     [ ask nearby-uninfected
       [ if mask? = true
        [ if infection-chance > mask-effectiveness ;; Infect no mask-mask true
         [ set infected? true
           set nb-infected (nb-infected + 1)
         ]
       ]
        ...
        if mask? = false and vaccine? = false
        [ if random-float 100 > infection-chance ;; Infect no mask-no mask
         [ set infected? true
           set nb-infected (nb-infected + 1)
         ]
       ]
...
to calculate-r0
  ...
  let susceptible-t
   (initial-people + initial-people-mask + initial-people-vaccine) -
    count turtles with [ infected? ] -
  ...
  if(initial-people + initial-people-mask + initial-people-vaccine) - susceptible-t != 0 and susceptible-t != 0
  [
    ;; This is derived from integrating dI / dS = (beta*SI - gamma*I) / (-beta*SI):
    set r0 (ln (s0 / susceptible-t) /((initial-people + initial-people-mask + initial-people-vaccine) - susceptible-t))
...
1614
766
Cumulative percentages of non Mask-Population
Days
% Total Population
0.0
10.0
...
"" ""
PENS
"% of total infected" 1.0 0 -2674135 true "" "plot (((count turtles with [ infected? and not mask? ]) / (initial-people)) * 100)"
"% of total non infected" 1.0 0 -10899396 true "" "plot ((count turtles with [ not infected? and not mask? ] / (initial-people)) * 100)"
"% of cured - non infected" 1.0 0 -13345367 true "" "plot ((count turtles with [ cured? and not mask? ] / (initial-people)) * 100)"
"% of uninfected - non infected" 1.0 0 -16777216 true "" "plot ((count turtles with [ not cured? and not mask? ] / (initial-people)) * 100)"
...
499
459
Cumulative percentages of Populations
Days
% Total Population
0.0
10.0
...
"" ""
PENS
"% of total infected " 1.0 0 -2674135 true "" "plot (((count turtles with [ infected? ]) / (initial-people + initial-people-mask)) * 100)"
"% of total non infected" 1.0 0 -10899396 true "" "plot ((count turtles with [ not infected? ] / (initial-people + initial-people-mask)) * 100)"
"% of cured - non infected" 1.0 0 -13345367 true "" "plot ((count turtles with [ cured? ] / (initial-people + initial-people-mask)) * 100)"
"% of uninfected - non infected" 1.0 0 -16777216 true "" "plot ((count turtles with [ not cured? ] / (initial-people + initial-people-mask)) * 100)"
...
1614
457
Cumulative percentages of Mask-Population
Days
% Total Population
0.0
10.0
...
"" ""
PENS
"% of total infected" 1.0 0 -2674135 true "" "plot (((count turtles with [ infected? and mask? ]) / (initial-people-mask)) * 100)"
"% of total non infected" 1.0 0 -10899396 true "" "plot ((count turtles with [ not infected? and mask? ] / (initial-people-mask)) * 100)"
"% of cured - non infected" 1.0 0 -13345367 true "" "plot ((count turtles with [ cured? and mask? ] / (initial-people-mask)) * 100)"
"% of uninfected - non infected" 1.0 0 -16777216 true "" "plot ((count turtles with [ not cured? and mask? ] / (initial-people-mask)) * 100)"
...
922
771
Cumulative percentages of Vaccine-Population
Days
% of Total Population
0.0
10.0
...
"" ""
PENS
"% of total vaccine" 1.0 0 -2674135 true "" "plot (((count turtles with [ infected? and vaccine? ]) / (initial-people-vaccine)) * 100)"
"% of total non infected" 1.0 0 -10899396 true "" "plot ((count turtles with [ not infected? and vaccine? ] / (initial-people-vaccine)) * 100)"
...
"" ""
PENS
"default" 1.0 0 -2674135 true "" "plot r0"
...
@#$#@#$#@
<experiments>
  <experiment name="Run100NonMaskVaccine" repetitions="100" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    ...
    <metric>count turtles with [ cured? ]</metric>
    <metric>count turtles with [ not cured? ]</metric>
    ...
    <enumeratedValueSet variable="initial-people-mask">
      <value value="0"/>
    </enumeratedValueSet>
  ...
  <experiment name="Run50_50NonMaskToMask" repetitions="100" sequentialRunOrder="false" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>count turtles</metric>
    <enumeratedValueSet variable="average-recovery-time">
  ...
  <experiment name="Run100Mask" repetitions="100" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>count turtles</metric>
    <enumeratedValueSet variable="average-recovery-time">
  ...
  <experiment name="Run75Mask_25NonMaskToMask" repetitions="100" sequentialRunOrder="false" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>count turtles</metric>
    <enumeratedValueSet variable="average-recovery-time">
  ...
  <experiment name="Run25Mask_75NonMaskToMask" repetitions="100" sequentialRunOrder="false" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>count turtles</metric>
    <enumeratedValueSet variable="average-recovery-time">
  ...
  <experiment name="RUN1" repetitions="100" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <metric>count turtles</metric>
    <metric>r0</metric>
    ...
    <enumeratedValueSet variable="initial-people-vaccine">
      <value value="0"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="initial-people-mask">
      <value value="0"/>
  ...
  <experiment name="10to10" repetitions="100" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <metric>count turtles</metric>
    <metric>r0</metric>
  ...
  <experiment name="20to20" repetitions="100" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <metric>count turtles</metric>
    <metric>r0</metric>
  ...
  <experiment name="30to30" repetitions="100" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <metric>count turtles</metric>
    <metric>r0</metric>
  ...
  <experiment name="40to40" repetitions="100" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <metric>count turtles</metric>
    <metric>r0</metric>
  ...
  <experiment name="50to50" repetitions="100" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <metric>count turtles</metric>
    <metric>r0</metric>
